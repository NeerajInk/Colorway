     ...
     <div class="content-wrap">
          <div class="fullwidth">
               ...
               <div>
                    ...
                    <?php
												printf(__('By %2$s', 'colorway'),
													'meta-prep meta-prep-author',
													sprintf( '<a class="url fn n" href="%1$s" title="%2$s">%3$s</a>',
														get_author_posts_url( get_the_author_meta( 'ID' ) ),
														sprintf( esc_attr__( 'View all posts by %s', 'colorway' ), get_the_author() ),
														get_the_author()
													)
												);
											?>
                    <span>|</span>
                    <?php
												printf( __('Published %2$s', 'colorway'),
												...
												if ( wp_attachment_is_image() ) {
													...
													printf( __( 'Full size is %s pixels', 'colorway'),
														sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
															wp_get_attachment_url(),
															esc_attr( __('Link to full-size image', 'colorway') ),
															$metadata['width'],
															$metadata['height']
														)
													);
